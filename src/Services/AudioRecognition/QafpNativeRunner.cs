    ...
    private static void EnsureInitialized()
    {
        ...
        lock (_lock)
        {
            ...
            if (!isArm64)
            {
                ...
                {
                    AppLogger.Force("QafpNativeRunner", "非 ARM64 平台未检测到 qemu-aarch64-static / qemu-aarch64，关闭优图识曲接口");
                    _initialized = true;
                    return;
                }
            }
            ...
            // 环境变量
            var envDir = Environment.GetEnvironmentVariable("QMTUI_QAFP_RUNNER_DIR") ??
                         Environment.GetEnvironmentVariable("QQMUSIC_QAFP_RUNNER_DIR");
            if (!string.IsNullOrEmpty(envDir)) candidates.Add(envDir);

            // 系统级共享目录 (包管理器安装标准路径)
            candidates.Add("/usr/share/qmtui/qafp");
            candidates.Add("/usr/local/share/qmtui/qafp");
            ...
            {
                candidates.Add(Path.Combine(userHome, ".local", "share", "qmtui", "qafp"));
                candidates.Add(Path.Combine(userHome, ".local", "share", "qmtui", "qafp"));
            }
            ...
            candidates.Add(Path.Combine(cwd, "qafp"));
            ...
            {
                AppLogger.Force("QafpNativeRunner", "未找到可运行的官方 QAFP 环境，关闭优图识曲接口");
            }
