...
class ElectronMusicPlayer {
  constructor() {
    ...
    // 主题与莫奈取色状态
    this.themeMode = localStorage.getItem('qmtui_web_theme') || 'system';
  ...
  togglePlay() {
    // 关键防御：若界面显示为播放中但底层音频实际由于自动播放限制处于暂停状态，用户点击播放键应直接恢复播放
  ...
  updateMediaSessionMetadata(song) {
    ...
    try {
      navigator.mediaSession.metadata = new MediaMetadata({
        title: song.title || 'qmtui',
        artist: song.artist || '未知歌手',
        album: song.album || 'qmtui',
        artwork: [
  ...
  setupTheme() {
    ...
    const cycleTheme = (e) => {
      ...
      localStorage.setItem('qmtui_web_theme', this.themeMode);
